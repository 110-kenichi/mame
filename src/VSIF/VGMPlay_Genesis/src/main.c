...
int main() {
  ...
  if (UART_MODE == TRUE) {
    ...
    if(((*port2) & 1) == 0)
    {
      VDP_drawText("*PLEASE CONNECT FTDI2XX TO MD*", 0, 4);
      while(((*port2) & 1) == 0);
      VDP_clearText(0, 4, 40);
    }

    VDP_drawText("FTDI2XX MODE READY TO PLAY.", 0, 2);
    VDP_drawText("*PLEASE RESET AFTER CONNECTION*", 0, 4);
    VDP_drawText("*TO AVOID INCORRECT SOUNDS*", 0, 5);

    VDP_drawText("-CONNECT P2 PORT PIN1-4 TO FTDI2XX.", 0, 7);
    VDP_drawText("-CONNECT P2 PORT PIN8 TO FTDI2XX GND.", 0, 8);

    VDP_drawText("  1,2,3,4 -> FTDI2XX TX,RX,RTS,CTS ", 0, 10);
    VDP_drawText(" ___________        ", 0, 11);
    VDP_drawText(" \\* * * * o/        ", 0, 12);
    VDP_drawText("  \\o o * o/         ", 0, 13);
    VDP_drawText("   -------          ", 0, 14);
    VDP_drawText("       8  -> FTDI2XX GND", 0, 15);
