...
#ifdef ALLOC_PRAGMA
#pragma alloc_text (PAGE, CMI8738OPL3QueueInitialize)
#endif

NTSTATUS
CMI8738OPL3QueueInitialize(
...
--*/
{
	...
	queueConfig.EvtIoDeviceControl = CMI8738OPL3EvtIoDeviceControl;
	queueConfig.EvtIoStop = CMI8738OPL3EvtIoStop;
...
--*/
{
	...
	//DbgPrint("CMI8738OPL3 Queue 0x%p, Request 0x%p OutputBufferLength %d InputBufferLength %d IoControlCode %d",
	//	Queue, Request, (int)OutputBufferLength, (int)InputBufferLength, IoControlCode);
...

	{
		...
		{
			//DbgPrint("CMI8738OPL3 IOCTL_WRITE_DATA_TO_PORT requested");
			...
			if (NT_SUCCESS(status))
			{
				if (InputBufferLength == 2)
				{
					...
					if (0 <= portOfst && portOfst < 4)
					{
						...
						PUCHAR portAdrs = (PUCHAR)ULongToPtr(deviceContext->IoBaseAddress + 0x50 + portOfst);
		...
		{
			//DbgPrint("CMI8738OPL3 IOCTL_READ_DATA_TO_PORT requested");
	...
	//DbgPrint("CMI8738OPL3 Queue End(stat=%x)", status);
	WdfRequestComplete(Request, status);
	return;
}

VOID
CMI8738OPL3EvtIoStop(
	_In_ WDFQUEUE Queue,
	_In_ WDFREQUEST Request,
