...
void gew_pcm_device::sound_stream_update(sound_stream& stream, stream_sample_t** inputs, stream_sample_t** outputs, int32_t samples)
{
...
int16_t gew_pcm_device::clamp_to_int16(int32_t value)
{
	if (value < -32768)
	{
		return -32768;
...
void gew_pcm_device::rom_bank_updated()
{
	m_stream->update();
}
